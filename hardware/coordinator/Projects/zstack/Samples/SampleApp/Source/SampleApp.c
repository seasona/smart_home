...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    if(LIGHT==1)
           { 
               LED1=1;
               HalUARTWrite(0,"0",1);     //串口
            }
        else 
            {
               LED1=0;
               HalUARTWrite(0,"1",1);       //串口
            }
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
 ...
 /*       HalUARTWrite(0,"$",1);
    if(LIGHT==1)
     ...
     HalUARTWrite(0,"#",1);
     int i=pkt->cmd.Data[2]-'0';
     int j=pkt->cmd.Data[3]-'0';
     if(i>=7 && j>=0)
               LED2=1;
        else 
               LED2=0;
